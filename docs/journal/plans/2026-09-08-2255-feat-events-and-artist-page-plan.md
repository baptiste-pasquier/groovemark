...
- R17. From `md` up, the artists tab is a table sortable by any of its columns: artist, mixes, moments, starred moments, performances, most recent verdict, and date last seen. Its most-recent-verdict column shows the most recent rated verdict -- the same value R14's leading summary shows -- and reports unrated only when no performance of that artist carries a verdict. Its date-last-seen column reports the most recent performance, rated or not, so a row can carry a verdict from an earlier night than the date beside it.
...
- **The artist page and the artists tab report the same leading verdict: the most recent rated one.** An unrated latest night would otherwise silence the page's summary exactly when it is being consulted, and a column that disagreed with the page it links to would read as a defect rather than as a rule. The residual asymmetry is narrower and deliberate: the date-last-seen column follows the most recent performance, so a row can show a verdict from an earlier night than the date beside it. (session-settled: user-approved -- chosen over the column reporting the latest performance's verdict as it stands: two surfaces contradicting each other about the same performer costs more than one date-versus-verdict mismatch inside a row.) Governs R14, R17.
...
- AE14. **Covers R17.** Given a performer whose latest performance is unrated and whose previous one carries two stars, when the artists tab is sorted by most recent verdict, then their row reports two stars -- the same value their page leads with per R14 -- while their date-last-seen column still reports the later, unrated performance.
...
- AE19. **Covers R17.** Given a performer seen live with no verdict on any performance, when the artists tab is opened, then their most-recent-verdict column reports unrated; and given a performer with no performance at all, that column reports no verdict rather than unrated.
...
**R17** was **amended** after the review, at the user's direction, and **AE14** with it: its most-recent-verdict column now reports the most recent rated verdict rather than the latest performance's verdict as it stands, so the column and the artist page it links to can no longer disagree. The deliberate divergence the earlier wording relied on is gone; what remains is the date-versus-verdict mismatch inside a row, which **AE19** and the Key Decision both name. The change touches no other requirement, and the units that carried the old rule -- U1's aggregation, U10's column, U14's display owner and KTD16 -- were reworded to match.
...
- **Approach:**
  ...
  4. Write the aggregation the artists table needs: per artist, a performance count, the most recent rated verdict -- read from step 3's selector rather than derived a second time, which is what keeps R17's column and R14's page from diverging -- and the date of the most recent performance, rated or not (R17, KTD16).
...
- **Test scenarios:**
  ...
  - Per-artist aggregation returns the same verdict the page selector returns over the same performances, including when the latest performance is unrated. _Covers AE14._
  - Aggregation's date last seen follows the most recent performance while its verdict comes from the most recent rated one, so the two disagree within a row when the latest night is unrated. _Covers AE14._
  - Aggregation reports unrated for an artist whose performances carry no verdict at all, and reports the absent case for an artist with no performance. _Covers AE19._
...
- **Approach:**
  1. Build the read-only verdict display consumed by the event card, the artist page and the artists table, reading U1's ordering and labels (KTD16). The repo inlines such a marker at one call site in `FavoriteCard.vue`; at four it drifts, and R17 now requires the artist page and the artists table to render the same value for the same performer.
...
- **Test scenarios:**
  ...
  - A row whose latest performance is unrated reports the earlier rated verdict, the same value that artist's page leads with, while its date-last-seen cell reports the later performance. _Covers AE14._
  - A row for an artist whose performances carry no verdict reports unrated, and a row for an artist with no performance reports no verdict. _Covers AE19._
