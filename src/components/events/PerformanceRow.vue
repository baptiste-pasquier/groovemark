...
<template>
  <div class="performance-row flex items-start gap-2">
    ...
    <VerdictPicker v-model="selectedVerdict" :artist-name="artistName" class="pt-2" />
    <!-- Removing a row asks for nothing: the whole event is what saves or
         fails, so nothing is lost until the save (R8). -->
    <button
      type="button"
      class="remove-performance-btn px-1 pt-1 text-xl font-bold text-red-500 hover:text-red-700 focus:ring-2 focus:ring-red-400 focus:outline-none"
