...
- Closed anonymous account creation on the PocketBase `users` collection. Its create rule had been left at the PocketBase default, an empty rule that means "anyone", so a public instance accepted `POST /api/collections/users/records` from a stranger and handed them an account. A migration now admits only PocketBase's internal OAuth2 sign-up request (`@request.context = "oauth2"`), which leaves Google sign-in -- including a first-ever one on a clean instance -- working unchanged.
...
- Widened the search field on desktop, and let it run the full width on a phone so it reaches the
  same edge as the create button below it.
